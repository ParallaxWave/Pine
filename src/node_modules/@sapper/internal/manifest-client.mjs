// This file is generated by Sapper — do not edit it!
// webpack does not support export * as root_comp yet so do a two line import/export
import * as root_comp from '../../../routes/_layout.svelte';
export { root_comp };
export { default as ErrorComponent } from '../../../routes/_error.svelte';

export const ignore = [/^\/clusters\/create\.json$/, /^\/blog\.json$/, /^\/blog\/([^/]+?)\.json$/, /^\/user\/getChats\.json$/, /^\/user\/register\.json$/, /^\/user\/verify\.json$/];

export const components = [
	{
		js: () => import("../../../routes/index.svelte")
	},
	{
		js: () => import("../../../routes/create.svelte")
	},
	{
		js: () => import("../../../routes/about.svelte")
	},
	{
		js: () => import("../../../routes/blog/index.svelte")
	},
	{
		js: () => import("../../../routes/blog/[slug].svelte")
	},
	{
		js: () => import("../../../routes/chat/index.svelte")
	},
	{
		js: () => import("../../../routes/join.svelte")
	}
];

export const routes = (d => [
	{
		// index.svelte
		pattern: /^\/$/,
		parts: [
			{ i: 0 }
		]
	},

	{
		// create.svelte
		pattern: /^\/create\/?$/,
		parts: [
			{ i: 1 }
		]
	},

	{
		// about.svelte
		pattern: /^\/about\/?$/,
		parts: [
			{ i: 2 }
		]
	},

	{
		// blog/index.svelte
		pattern: /^\/blog\/?$/,
		parts: [
			{ i: 3 }
		]
	},

	{
		// blog/[slug].svelte
		pattern: /^\/blog\/([^/]+?)\/?$/,
		parts: [
			null,
			{ i: 4, params: match => ({ slug: d(match[1]) }) }
		]
	},

	{
		// chat/index.svelte
		pattern: /^\/chat\/?$/,
		parts: [
			{ i: 5 }
		]
	},

	{
		// join.svelte
		pattern: /^\/join\/?$/,
		parts: [
			{ i: 6 }
		]
	}
])(decodeURIComponent);

if (typeof window !== 'undefined') {
	import("/home/crypticwave/Pine/node_modules/sapper/sapper-dev-client.js").then(client => {
		client.connect(10000);
	});
}